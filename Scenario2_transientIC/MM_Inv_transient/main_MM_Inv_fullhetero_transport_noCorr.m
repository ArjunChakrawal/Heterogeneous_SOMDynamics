...
load('Spatial_field\ksmm_inv_hetero.mat');
load('Spatial_field\Km_inv_hetero.mat');
...
for a1 = 1:length(alpha)
    ...
    ksmm_inv = ksmm_inv_hetero;
    kb = ones(Nx, Ny) .* kb;
    Km_inv = Km_inv_hetero;
    ...
    save(['output\', 'Ph_mm_inv_fullhetero_noCorr_', num2str(alpha(a1)), '.mat'])
end
